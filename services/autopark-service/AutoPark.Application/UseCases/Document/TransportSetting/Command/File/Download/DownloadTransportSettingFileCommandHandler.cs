...
public class DownloadTransportSettingFileCommandHandler :
    IRequestHandler<DownloadTransportSettingFileCommand, (byte[], string)?>
{
    private readonly IStorageAsyncService _storageAsyncService;
    private readonly IReadEfCoreContext _context;
    public DownloadTransportSettingFileCommandHandler(IStorageAsyncService storageAsyncService, IReadEfCoreContext context)
    {
    ...
    public async Task<(byte[], string)?> Handle(
        DownloadTransportSettingFileCommand request,
        CancellationToken cancellationToken)
    {
        var entity = await _context.TransportSettingFiles
            .FirstOrDefaultAsync(x => x.Id == request.fileId, cancellationToken);

        StorageFile file;
        string document = FileStorageConst.TRANSPORT_SETTING_FILE;
