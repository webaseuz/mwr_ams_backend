...
public class CreateFuelCardCommandValidator :
    AbstractValidator<CreateFuelCardCommand>
{
    public CreateFuelCardCommandValidator()
    {
        ...
        RuleFor(x => x.BranchId)
            .GreaterThan(0)
            .LessThan(int.MaxValue)
            .Failure(ClientValidationExceptionHelper.InvalidLengthProperty(nameof(CreateFuelCardCommand.BranchId), 1, int.MaxValue));
        ...
        RuleFor(x => x.CardNumber)
            .NotEmpty()
            .Matches(@"^\d{4}\*{8}\d{4}$")
            .Failure(ClientValidationExceptionHelper.InvalidPropertyFormat(nameof(CreateFuelCardCommand.CardNumber)));
    }
}
