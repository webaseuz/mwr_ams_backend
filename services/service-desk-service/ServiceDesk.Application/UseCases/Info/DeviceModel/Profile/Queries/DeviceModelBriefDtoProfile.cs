...
public class DeviceModelBriefDtoProfile : Profile
{
    public DeviceModelBriefDtoProfile()
    {
        int lang = default;
        CreateMap<DeviceModel, DeviceModelBriefDto>()
            .ForMember(src => src.ShortName, conf => conf.MapFrom(ent => ent.Translates.AsQueryable()
            .FirstOrDefault(DeviceModelTranslate.GetExpr(TranslateColumn.short_name, lang)).TranslateText ?? ent.ShortName))
            .ForMember(src => src.FullName, conf => conf.MapFrom(ent => ent.Translates.AsQueryable()
            .FirstOrDefault(DeviceModelTranslate.GetExpr(TranslateColumn.full_name, lang)).TranslateText ?? ent.FullName))
