...
public class DevicePartBriefDtoProfile : Profile
{
    public DevicePartBriefDtoProfile()
    {
        int lang = default;
        CreateMap<DevicePart, DevicePartBriefDto>()
            .ForMember(src => src.ShortName, conf => conf.MapFrom(ent => ent.Translates.AsQueryable()
            .FirstOrDefault(DevicePartTranslate.GetExpr(TranslateColumn.short_name, lang)).TranslateText ?? ent.ShortName))
            .ForMember(src => src.FullName, conf => conf.MapFrom(ent => ent.Translates.AsQueryable()
            .FirstOrDefault(DevicePartTranslate.GetExpr(TranslateColumn.full_name, lang)).TranslateText ?? ent.FullName))
