...
public class WaitSparesServiceApplicationCommandHandler :
    IRequestHandler<WaitSparesServiceApplicationCommand, IHaveId<long>>
{
    ...
    public WaitSparesServiceApplicationCommandHandler(
        IMapProvider mapper,
    ...
    public async Task<IHaveId<long>> Handle(
        WaitSparesServiceApplicationCommand request,
        ...
        try
        {
            ...
            try
            {
                var testTemplate = await _context.NotificationTemplates
                    .FirstOrDefaultAsync(x => x.Key == NotificationTemplateConst.DOCUMENT_WAIT_SPARES, cancellationToken);

                if (testTemplate is null)
                    throw ClientLogicalExceptionHelper.NotificationTemplateNotFound(NotificationTemplateConst.DOCUMENT_WAIT_SPARES);
