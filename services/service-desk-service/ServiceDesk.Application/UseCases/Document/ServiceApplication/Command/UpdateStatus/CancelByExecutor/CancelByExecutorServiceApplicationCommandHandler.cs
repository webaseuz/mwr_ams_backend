...
public class CancelByExecutorServiceApplicationCommandHandler :
    IRequestHandler<CancelByExecutorServiceApplicationCommand, IHaveId<long>>
{
    ...
    public CancelByExecutorServiceApplicationCommandHandler(
    ...
    public async Task<IHaveId<long>> Handle(
        CancelByExecutorServiceApplicationCommand request, 
        ...
        try
        {
            ...
            try
            {
                var testTemplate = await _context.NotificationTemplates
                    .FirstOrDefaultAsync(x => x.Key == NotificationTemplateConst.DOCUMENT_CANCELED_BY_EXECUTOR, cancellationToken);

                if (testTemplate is null)
                    throw ClientLogicalExceptionHelper.NotificationTemplateNotFound(NotificationTemplateConst.DOCUMENT_CANCELED_BY_EXECUTOR);
                ...
                var message = _notificationService.GenerateMessageFromTemplate<StatusTemplateDto>(testTemplate,
                                new StatusTemplateDto(document.DocNumber,
                                                    ...
                                                    "Ijrochi tomonidan bekor qilingan"));
