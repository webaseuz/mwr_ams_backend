...
public class FinishByExecutorServiceApplicationCommandHandler :
    IRequestHandler<FinishByExecutorServiceApplicationCommand, IHaveId<long>>
{
    ...
    public FinishByExecutorServiceApplicationCommandHandler(
    ...
    public async Task<IHaveId<long>> Handle(
        FinishByExecutorServiceApplicationCommand request, 
        ...
        try
        {
            ...
            try
            {
                var testTemplate = await _context.NotificationTemplates
                    .FirstOrDefaultAsync(x => x.Key == NotificationTemplateConst.DOCUMENT_FINISHED_BY_EXECUTOR, cancellationToken);

                if (testTemplate is null)
                    throw ClientLogicalExceptionHelper.NotificationTemplateNotFound(NotificationTemplateConst.DOCUMENT_FINISHED_BY_EXECUTOR);
