...
public class OptimalRouteService : IOptimalRouteService
{
    ...
    private Task<double> CalculateDistanceAsync(Location start, Location end)
    {
    ...
    private double DegreeToRadian(double gradus) => gradus / 180 * Math.PI;

    private async Task<List<Location>> GetOptimalWithLimitAsync(Location start, Location end, List<Location> locations)
    {
    ...
    private async Task<Location> GetNerarestNeighbourAsync(Location start, List<Location> all)
    {
        var len = double.MaxValue;
