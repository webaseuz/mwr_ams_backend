...
public class TokenUserInfoDtoProfile : CultureProfile
{
    public TokenUserInfoDtoProfile()
    {
        ...
        CreateMap<User, UserInfoDto>()
            ...
            .ForMember(src => src.DepartmentName, exp => exp.MapFrom(ent => ent.Department.Translates.AsQueryable().FirstOrDefault(DepartmentTranslate.GetExpr(TranslateColumn.short_name, cultureId)).TranslateText ?? ent.Department.ShortName))
            .ForMember(src => src.PositionName, exp => exp.MapFrom(ent => ent.Position.Translates.AsQueryable().FirstOrDefault(PositionTranslate.GetExpr(TranslateColumn.short_name, cultureId)).TranslateText ?? ent.Position.ShortName));
    }
}
