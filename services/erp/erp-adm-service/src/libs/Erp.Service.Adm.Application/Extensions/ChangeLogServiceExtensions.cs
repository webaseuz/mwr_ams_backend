...
public static class ChangeLogServiceExtensions
{
        ...
        Assembly modelsAssembly = null)
    {
        ...
        Assembly coreModelsAssembly = typeof(PersonDto).Assembly;

        // IDocumentEntity<TId> ni implement qilgan barcha classlarni top
        var coreDocumentEntityTypes = coreModelsAssembly.GetTypes()
            .Where(t => t.IsClass &&
                       !t.IsAbstract &&
                       t.GetInterfaces().Any(i => i.IsGenericType &&
                                                  i.GetGenericTypeDefinition() == typeof(ISysEntity<>)))
        ...
        documentEntityTypes.AddRange(coreDocumentEntityTypes);
        ...
        params Type[] entityTypes)
    {
        foreach (var entityType in entityTypes)
        {
            ...
            var documentEntityInterface = entityType.GetInterfaces()
                .FirstOrDefault(i => i.IsGenericType && i.GetGenericTypeDefinition() == typeof(ISysEntity<>));

            if (documentEntityInterface == null)
            {
                Console.WriteLine($"  ⚠ Skipped: {entityType.Name} (doesn't implement ISysEntity)");
                continue;
            }
