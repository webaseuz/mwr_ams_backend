...
public class StorageFileInfo : IStorageFileInfo
{
    ...
    public StorageFileInfo(Guid fileId,
                           string fileName,
                           long fIleSize) : this(fileId, fileName)
    {
        FileSize = fIleSize;
    }
...
public class StorageFile : StorageFileInfo
{
    ...
    public StorageFile(IStorageFileInfo storageFileInfo, Stream stream)
        : this(storageFileInfo.FileId,
               storageFileInfo.FileName,
               stream,
               storageFileInfo.FileSize)
    {

    }

    public StorageFile(Guid fileId,
                       string fileName,
                       Stream stream,
                       long fileSize = 0,
                       string fileMimeType = "")
    ...
    public void Validate()
    {
        if (FileName.NullOrEmpty() || FileName.IndexOfAny(Path.GetInvalidFileNameChars()) > 0 ||
            ...
            throw new Exception($"Filename ({FileName}) not valid");
