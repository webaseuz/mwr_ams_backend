    ...
    : ICultureHelper
{
    ...
    protected abstract IEnumerable<CultureModel> GetCultures();
    ...
    public IReadOnlyCollection<CultureModel> Cultures
    {
        ...
        {
            if (cultures == null)
                cultures = Array.AsReadOnly(GetCultures()
                                                .ToArray());
    ...
    private void Init()
    {
        try
        {
            foreach (var (text, dictionary) in from data in JsonConvert.DeserializeObject<ExpandoObject>(File.ReadAllText(this.config.JsonFilePath))
                                               select (data.Key,
